...
static char *tmkNoIns = "<Missing insertstring>";

#define tmkMESSZ 2
typedef char *tmkMsgs[tmkMESSZ];
    ...
    if (mstr[msidx] == ESCAPECHAR && mstr[msidx+1]) {
      ...
      if (iptr > 0) {
        ...
        for (s = istr; iptr > 1 && *s; iptr--) {
          for (; *s && *s != tmkSEPARATOR; s++);
          if (*s) s++;
	}

        /* copy insertion string */
        for (; *s && *s != tmkSEPARATOR; rb[rbidx++] = *(s++));
      }
    } else {
      /* ordinary character */
      rb[rbidx++] = mstr[msidx++];
...
static void prsrcl(
     ...
     tmkSev errflg 		/* IN - Set of errors on this line */
)
{
...
/*----------------------------------------------------------------------
  ...
  msarr[]. NOTE the difference to tmkMsg()!!
...
static void prpack(
  ...
  int i;			/* Counter index */
  tmkSev msgflg; /* Set of msg severities found */
  ...
  if (inset(msgflg, sevPAGE)) {
	...
	break;
    tmkSkipLines(mdarr[msarr[i].ref].code); /* code contains #lines */
  }
...
static void liLog(
  ...
  if (istrs != NULL) {
    ...
    if (!mdarr[count.msgs].insert) {
      error("Out of memory!");
      mdarr[count.msgs].insert = tmkNoIns; 
    } else
 ...
 */
static void liInit(
     tmkTyp typ,	/* IN - Listing type */
     tmkSev sevs,	/* IN - Severities to include */
...
void tmkLiInit(
     ...
     tmkMessages msect	/* IN - Message sector */
) {
...
void tmkLog(
    ...
    tmkSev sev,		/* IN - Severity code */
    char istrs[]			/* IN - Insert strings */
) {
  
  /* Check phase */
  if (phase != PH_COLL) {
    error("tmkLog(): Phase error.");
    return;
  }
  ...
  if (pos != NULL) {
    ...
    if (pos->col < 0) {
      error("tmkLog(): Source position column < 0.");
      pos->col = 0;
    }
    if (pos->line < 0) {
      error("tmkLog(): Source position line < 0.");
      pos->line = 0;
    }
    if (pos->line == 0) {
      pos->col = 0;
  ...
  /* Log the message */
  liLog(pos, NULL, ecode, sev, istrs);
...
void tmkLogv(
  ...
  tmkLog(pos, ecode, sev, errorstring);
  free(errorstring);
}
...
void tmkSkipLines(
  ...
  if (phase != PH_RETR)
    error("tmkSkipLines(): Phase error.");
...
void tmkLiTerminate(
  ...
  for (i=0; i < count.msgs; i++)
    if (mdarr[i].insert != tmkNoIns && mdarr[i].insert != NULL)
      free(mdarr[i].insert);
