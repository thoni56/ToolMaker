...
typedef struct {
    ContRec cont[3];		/* Continuation function */
...
typedef struct  {
    UByte1 act[5];		/* Action table */
    UByte1 actCheck[6];	/* Row Displacement Check Vector*/
    UByte1 actRix[3];		/* Action row mapping */
...
\* ------------------ */
typedef  struct {
    UByte1 gto[2];		/* Goto table */
    UByte1 gtoRix[3];		/* Goto row mapping */
    UByte1 lhs[3];		/* Left hand side symbols */
    UByte1 rhsz[3];		/* Size of the RHS */
} NonTermTab;
...
\* ------------------------------------- */
short codeStkP;			/* Stack pointer */
TmToken codeSySt[PmStkMax];	/* Symbol stack */
codeGrammar codeSeSt[PmStkMax];	/* Semantic stack */
...
static UByte1 scIndex[2 + 1]; /* Converts scanner code to internal value */
...
extern void codeRPoi(	/* Error recovery - restart point  */
TmToken *sym			/* IN the restart symbol */
);
extern void codeISym(	/* Error recovery - insert symbol  */
...
extern void codeDSym(	/* Error recovery - delete symbol  */
TmToken *sym,			/* IN terminal code number */
char *sstr,			/* IN terminal string */
char *pstr			/* IN terminals print string */
);
extern void codeMess(	/* Error recovery - error message  */
...
{
    codeMess(token, HALT_REC, eClass, severity);
 ...
 *----------------------------------------------------------------------------
 */
#define MOVETOKEN codeSySt[codeStkP] = *token
#define pushStatTok()\
{\
    if (++codeStkP < PmStkMax) {\
    parsStk[codeStkP] = currstat;\
    MOVETOKEN;\
    } else {\
...
)
{
    ...
    codeDSym(token, voc[t].name, voc[t].psym);
{
    if (la_ptr < la_end) {
    la_ptr++;
...
{
    codeISym(voc[isym - TermMin].scc, voc[isym - TermMin].name,
...
)
{
    ...
    codeMess(&errdescr.err_tok, errdescr.method, errdescr.eClass,
           errdescr.severity);
    } else {
    codeMess(&errdescr.err_tok, errdescr.method, SYN_ERR, errdescr.severity);
    }/*if*/
    errdescr.res_tok = *err_symb;
    errdescr.res_tok.Code = voc[errdescr.res_tok.Code - TermMin].scc;
    codeRPoi(&errdescr.res_tok);
}/*recover()*/


/*----------------------------------------------------------------------------
 * codeParse - Parser routine, entry point
 *----------------------------------------------------------------------------
 */
void codeParse(void)
{
    paInit();
{
    ...
    *token = la_buff[la_ptr];
    ...
    case ERROR_ACT:
        recover(parsStk, &codeStkP, token, &currstat);
        if (codeStkP == 0) endparse = PMTRUE;
        break;
        ...
        pushStatTok();
{
    ...
    *token = la_buff[la_ptr];
    ...
    case REDUCE_ACT:
      reduce:
        do {
        codeStkP -= gto.rhsz[arg - ProdMin];
        codePaSema(arg);
        gtoTbl(parsStk[codeStkP], gto.lhs[arg - ProdMin], &action, &arg);
        pushStat();
        } while (action != SHIFT_ACT);
