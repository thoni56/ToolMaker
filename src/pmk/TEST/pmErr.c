...
)
{
	...
	tmLog(&(sym->srcp), 102, sev, "Unknown Token");
	break;
	...
	case 1:
	    /* Insert */
	    tmLog(&(sym->srcp), 101, sev, insStr);
	    break;

	case 2:
	    /* Delete */
	    tmLog(&(sym->srcp), 102, sev, delStr);
	    break;

	case 3:
	    /* Replace */
	    delStr[strlen(delStr)+1] = '\0';
	    delStr[strlen(delStr)] = tmSEPARATOR; /* Separator */
	    strcat(delStr, insStr);
	    tmLog(&(sym->srcp), 103, sev, delStr);
	    break;

	case 4:
	    /* Stack backed up */
	    tmLog(&(sym->srcp), 104, sev, "");
	    break;

	case 5:
	    /* Syntax error, system halted */
	    tmLog(&(sym->srcp), 105, sev, "");
	    break;
	}
	break;
	...
	tmLog(&(sym->srcp), 106, sev, "");
	break;

    case 4:
	/* Parse table error */
	tmLog(&(sym->srcp), 107, sev, "");
	break;
    }

    if (method == 5) {
	/* System halted, output informational message */
	tmLog(&(sym->srcp), 108, sevINF, "");
    }
}/*pmMess()*/
