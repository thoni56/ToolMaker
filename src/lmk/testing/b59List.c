...
static char *b59NoIns = "<Missing insertstring>";

#define b59MESSZ 2
typedef char *b59Msgs[b59MESSZ];
...
static b59Msgs msg[] = {
    ...
    { "100   ", "Message with one insert string: \"%1\"" },
...
static b59Messages currMsect = (b59Messages)0;
...
static b59Typ lsttyp; /* Requested listing type */
static b59Sev lstsev; /* and severities */
...
static b59Sev maxsev;		/* Highest severity so far */
static b59Sev maxlocsev;	/* Highest local severity so far */
    ...
    if (mstr[msidx] == ESCAPECHAR && mstr[msidx+1]) {
      ...
      if (iptr > 0) {
        ...
        for (s = istr; iptr > 1 && *s; iptr--) {
          for (; *s && *s != b59SEPARATOR; s++);
          if (*s) s++;
	}

        /* copy insertion string */
        for (; *s && *s != b59SEPARATOR; rb[rbidx++] = *(s++));
      }
    } else {
      /* ordinary character */
      rb[rbidx++] = mstr[msidx++];
...
static void geterr(
     ...
     b59Sev *errflg /* OUT - set of severities found in line */
)
{
  /* initialize */
  *errflg = sevNONE; 
...
static void prsrcl(
     ...
     b59Sev errflg 		/* IN - Set of errors on this line */
)
{
...
/*----------------------------------------------------------------------
  ...
  msarr[]. NOTE the difference to b59Msg()!!
...
static void prpack(
  ...
  int i;			/* Counter index */
  b59Sev msgflg; /* Set of msg severities found */
  ...
  if (inset(msgflg, sevPAGE)) {
	...
	break;
    b59SkipLines(mdarr[msarr[i].ref].code); /* code contains #lines */
  }
...
static void liLog(
  ...
  if (istrs != NULL) {
    ...
    if (!mdarr[count.msgs].insert) {
      error("Out of memory!");
      mdarr[count.msgs].insert = b59NoIns; 
    } else
 ...
 */
static void liInit(
     b59Typ typ,	/* IN - Listing type */
     b59Sev sevs,	/* IN - Severities to include */
...
void b59LiInit(
     ...
     b59Messages msect	/* IN - Message sector */
) {
...
void b59Log(
  ...
  if (pos != NULL) {
    ...
    srcpos = *pos;
    if (srcp->col < 0) {
      error("b59Log(): Source position column < 0.");
      srcp->col = 0;
    }
    if (srcp->line < 0) {
      error("b59Log(): Source position line < 0.");
      srcp->line = 0;
    }
    if (srcp->line == 0) {
      srcp->col = 0;
...
void b59Logv(
  ...
  b59Log(pos, ecode, sev, errorstring);
  free(errorstring);
}
...
void b59SkipLines(
  ...
  if (phase != PH_RETR)
    error("b59SkipLines(): Phase error.");
...
void b59LiTerminate(
  ...
  for (i=0; i < count.msgs; i++)
    if (mdarr[i].insert != b59NoIns && mdarr[i].insert != NULL)
      free(mdarr[i].insert);
  free(mdarr);
  free(msarr);
