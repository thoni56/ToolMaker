...
static char *lmkNoIns = "<Missing insertstring>";

#define lmkMESSZ 2
typedef char *lmkMsgs[lmkMESSZ];
...
static lmkMessages currMsect = (lmkMessages)0;
...
static lmkTyp lsttyp;           /* Requested listing type */
static lmkSev lstsev;           /* and severities */
...
static lmkSev maxsev;           /* Highest severity so far */
static lmkSev maxlocsev;        /* Highest local severity so far */
    ...
    if (mstr[msidx] == ESCAPECHAR && mstr[msidx+1]) {
      ...
      if (iptr > 0) {
        ...
        for (s = istr; iptr > 1 && *s; iptr--) {
          for (; *s && *s != lmkSEPARATOR; s++);
          if (*s) s++;
    }

        /* copy insertion string */
        for (; *s && *s != lmkSEPARATOR; rb[rbidx++] = *(s++));
      }
    } else {
      /* ordinary character */
      rb[rbidx++] = mstr[msidx++];
...
static void geterr(
     ...
     lmkSev *errflg     /* OUT - set of severities found in line */
)
{
  /* initialize */
  *errflg = sevNONE;
...
static void prlin(
     ...
     LMBOOL cont,       /* IN - TRUE : equally sized line to follow on the same page */
     LMBOOL wrdwrp,     /* IN - TRUE : wrap after BLANK or COMMA if possible. */
...
static void prsrcl(
     ...
     lmkSev errflg              /* IN - Set of errors on this line */
)
{
...
/*----------------------------------------------------------------------
    ...
    msarr[]. NOTE the difference to lmkMsg()!!
...
static void prpack(
  ...
  int i;                        /* Counter index */
  lmkSev msgflg; /* Set of msg severities found */
  ...
  if (inset(msgflg, sevPAGE)) {
    ...
    break;
    lmkSkipLines(mdarr[msarr[i].ref].code); /* code contains #lines */
  }
...
static void liLog(
  ...
  if (istrs != NULL) {
    ...
    if (!mdarr[count.msgs].insert) {
      error("Out of memory!");
      mdarr[count.msgs].insert = lmkNoIns;
    } else
 ...
 */
static void liInit(
    lmkTyp typ,                 /* IN - Listing type */
    lmkSev sevs,                /* IN - Severities to include */
...
void lmkLiInit(
    ...
    lmkMessages msect           /* IN - Message sector */
) {
...
void lmkLog(
  ...
  if (pos != NULL) {
    ...
    srcpos = *pos;
    if (srcp->col < 0) {
      error("lmkLog(): Source position column < 0.");
      srcp->col = 0;
    }
    if (srcp->line < 0) {
      error("lmkLog(): Source position line < 0.");
      srcp->line = 0;
    }
    if (srcp->line == 0) {
      srcp->col = 0;
...
void lmkLogv(
  ...
  lmkLog(pos, ecode, sev, errorstring);
  free(errorstring);
}
 ...
 */
void lmkLiEnter(
    TmkSrcp *srcp,              /* IN - Where to include the file */
    TmkSrcp *start,             /* IN - First position to include from file */
 ...
 */
void lmkLiExit(
    TmkSrcp *srcp               /* IN - Where to return from an include file */
) {
...
void lmkSkipLines(
  ...
  if (phase != PH_RETR)
    error("lmkSkipLines(): Phase error.");
...
void lmkLiTerminate(
  ...
  for (i=0; i < count.msgs; i++)
    if (mdarr[i].insert != lmkNoIns && mdarr[i].insert != NULL)
      free(mdarr[i].insert);
  free(mdarr);
  free(msarr);
