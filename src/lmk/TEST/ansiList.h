#ifndef _ansiList_H_
#define _ansiList_H_
/*----------------------------------------------------------------------*\

	ansiList.h

	Header file for ListerMaker generated error message and listing
	handler

\*----------------------------------------------------------------------*/

#include "ansiCommon.h"


/* Insert string separator */
#define ansiSEPARATOR ((char)0xff)

/* Severity type and codes */
typedef int ansiSev;
#define sevNONE 0
#define sevOK   (1<<0)
#define sevINF  (1<<1)
#define sevWAR  (1<<2)
#define sevERR  (1<<3)
#define sevFAT  (1<<4)
#define sevSYS  (1<<5)

#define sevALL (sevOK|sevINF|sevWAR|sevERR|sevFAT|sevSYS)


/* Listing types */
typedef int ansiTyp;
#define liNONE  0
#define liSUM   (1<<0)		/* Summary */
#define liERR   (1<<1)		/* Erroneous source lines */
#define liOK    (1<<2)		/* Correct source lines */
#define liINCL  (1<<3)		/* Look also in PUSHed files */
#define liHEAD  (1<<4)		/* Heading */

#define liTINY (liSUM|liERR|liHEAD|liINCL)
#define liFULL (liTINY|liOK)

typedef enum ansiMessages {
    ansi_ENGLISH_Messages
} ansiMessages;


/* UNINITIALISED: */
/* Initialise the ansiLister System */
extern void ansiLiInit(char header[],
				 char src[],
				 ansiMessages msect);

/* COLLECTING: */
/* Log a message at a source position */
extern void ansiLog(TmSrcp *pos,
				int ecode,
				ansiSev sev,
				char *istrs);

/* Log a message at a source position using va_arg handling */
extern void ansiLogv(TmSrcp *pos,
				int ecode,
				ansiSev sev,
				...);

/* Turn listing completely off after a particular source position */
extern void ansiLiOff(TmSrcp *pos);

/* Turn listing on again at a particular source position */
extern void ansiLiOn(TmSrcp *pos);

/* Start reading an included file at a particular source position */
extern void ansiLiEnter(TmSrcp *pos,
				  TmSrcp *start,
				  char fnm[]);

/* Stop reading from an included file prematurely */
extern void ansiLiExit(TmSrcp *pos);

/* (Un)conditionally skip to a new page at a source position */
extern void ansiLiPage(TmSrcp *pos,
				 int lins);

/* Read worst severity logged so far */
extern ansiSev ansiSeverity(void);
extern ansiSev ansiLocSeverity(void);
extern void ansiResLocSeverity(void);

/* RETRIEVING: */
/* Create a listing of a selected type in a file or the screen */
extern void ansiList(char ofnm[],
				 int lins,
				 int cols,
				 ansiTyp typ,
				 ansiSev sevs);

/* Return the i'th formatted message, return 0 if not found */
extern int ansiMsg(int i,
			       TmSrcp *pos,
			       char *msg);

/* Print a string on a line in the output file */
extern void ansiLiPrint(char str[]);

/* (Un)conditionally skip to a new page in the output file */
extern void ansiSkipLines(int lins);

/* Terminate the ansiLister system */
extern void ansiLiTerminate(void);


#endif

