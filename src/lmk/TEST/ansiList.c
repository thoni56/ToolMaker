...
static char *ansiNoIns = "<Missing insertstring>";

#define ansiMESSZ 2
typedef char *ansiMsgs[ansiMESSZ];
...
static ansiMessages currMsect = (ansiMessages)0;
...
static ansiTyp lsttyp; /* Requested listing type */
static ansiSev lstsev; /* and severities */
...
static ansiSev maxsev;		/* Highest severity so far */
static ansiSev maxlocsev;	/* Highest local severity so far */
    ...
    if (mstr[msidx] == ESCAPECHAR && mstr[msidx+1]) {
      ...
      if (iptr > 0) {
        ...
        for (s = istr; iptr > 1 && *s; iptr--) {
          for (; *s && *s != ansiSEPARATOR; s++);
          if (*s) s++;
	}

        /* copy insertion string */
        for (; *s && *s != ansiSEPARATOR; rb[rbidx++] = *(s++));
      }
    } else {
      /* ordinary character */
      rb[rbidx++] = mstr[msidx++];
...
static void geterr(
     ...
     ansiSev *errflg /* OUT - set of severities found in line */
)
{
  /* initialize */
  *errflg = sevNONE; 
...
static void prsrcl(
     ...
     ansiSev errflg 		/* IN - Set of errors on this line */
)
{
...
/*----------------------------------------------------------------------
  ...
  msarr[]. NOTE the difference to ansiMsg()!!
...
static void prpack(
  ...
  int i;			/* Counter index */
  ansiSev msgflg; /* Set of msg severities found */
  ...
  if (inset(msgflg, sevPAGE)) {
	...
	break;
    ansiSkipLines(mdarr[msarr[i].ref].code); /* code contains #lines */
  }
...
static void liLog(
     ...
     ansiSev sev,	/* IN - severity to log */
  ...
  if (istrs != NULL) {
    ...
    if (!mdarr[count.msgs].insert) {
      error("Out of memory!");
      mdarr[count.msgs].insert = ansiNoIns; 
    } else
 ...
 */
static void liInit(
     ansiTyp typ,	/* IN - Listing type */
     ansiSev sevs,	/* IN - Severities to include */
...
void ansiLiInit(
     ...
     ansiMessages msect	/* IN - Message sector */
) {
...
void ansiLog(
  ...
  if (pos != NULL) {
    ...
    srcpos = *pos;
    if (srcp->col < 0) {
      error("ansiLog(): Source position column < 0.");
      srcp->col = 0;
    }
    if (srcp->line < 0) {
      error("ansiLog(): Source position line < 0.");
      srcp->line = 0;
    }
    if (srcp->line == 0) {
      srcp->col = 0;
...
void ansiLogv(
  ...
  ansiLog(pos, ecode, sev, errorstring);
  free(errorstring);
}
...
void ansiSkipLines(
  ...
  if (phase != PH_RETR)
    error("ansiSkipLines(): Phase error.");
...
void ansiLiTerminate(
  ...
  for (i=0; i < count.msgs; i++)
    if (mdarr[i].insert != ansiNoIns && mdarr[i].insert != NULL)
      free(mdarr[i].insert);
  free(mdarr);
  free(msarr);
